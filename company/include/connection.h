    ...
    try {
      if (session_ == nullptr) {
        session_ = std::shared_ptr<mysqlx::Session>(new mysqlx::Session(
            ...
            mysqlx::SessionOption::PORT, 33060, mysqlx::SessionOption::USER,
            "root", mysqlx::SessionOption::PWD, "root",
            mysqlx::SessionOption::DB, "company"));
        schema_ = std::shared_ptr<mysqlx::Schema>(
            new mysqlx::Schema(session_->getDefaultSchema()));
      }

    } catch (...) {
      throw "数据库未正确连接";
    }
    return std::shared_ptr<mysqlx::Table>(
        new mysqlx::Table(schema_->getTable(name)));
  }
