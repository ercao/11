...
shared_ptr<System> SystemDao::login(std::string username, std::string password) {
...
shared_ptr<System> SystemDao::getSystemInfo(string username) {
	auto row_result = table_->select("username", "password", "unix_timestamp(login_time)").where("`username`=" + Util::quote(username)).execute();
	if (row_result.count() <= 0) {
		throw std::string("用户不存在");
	}
	mysqlx::Row row = row_result.fetchOne();

	return shared_ptr<System>(new System(Util::utf8_to_string(row[0].get<string>()), Util::utf8_to_string(row[1].get<string>()), row[2]));
}
