...
ResultEntity<PageInfo<Role>> RoleService::getList(const string &page_no, const string &page_size) {
	...
	try {
		...
		return ResultEntity<PageInfo<Role>>::successResultWidthData("成功获取到" + to_string(result->data_.size()) + "条数据", result);
