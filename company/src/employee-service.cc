...
ResultEntity<PageInfo<Employee>>
EmployeeService::getPageInfo(string page_no, string page_size) {
  ...
  try {
    ...
    return ResultEntity<PageInfo<Employee>>::successResultWidthData(
        "成功获取到" + to_string(result->data_.size()) + "条数据", result);
  } catch (const string &e) {
    return ResultEntity<PageInfo<Employee>>::faildResult(GETLIST_FAILD_PREFIX +
                                                         e);
...
ResultEntity<Employee> EmployeeService::getOne(string id) {
  ...
  try {
    ...
    fillDepartmentAndRole(employee);
    return ResultEntity<Employee>::successResultWidthData("获取数据成功",
                                                          employee);
  } catch (const string &e) {
...
ResultEntity<int> EmployeeService::add(Employee &object) {
  ...
  try {
    ...
    if (object.gender() == "" && object.gender() != "男" &&
        object.gender() != "女") {
      throw string("性别不能为空");
    }
...
ResultEntity<int> EmployeeService::edit(Employee &object) {
  ...
  try {
    ...
    if (object.gender() == "" ||
        (object.gender() != "男" && object.gender() != "女")) {
    ...
    // 部门
    if ((object.departmentId() >= 100 &&
         object.departmentId() != employee->departmentId()) ||
...
ResultEntity<double> EmployeeService::getMonthSalary(Employee &employee) {
  ...
  try {
    ...
    double month_salary =
        role_dao_->getMonthSalary(employee.id(), employee.roleId());
    Connection::commit();
    return ResultEntity<double>::successResultWidthData(
        "获得月工资成功", shared_ptr<double>(new double(month_salary)));
  } catch (const string &e) {
    Connection::rollback();
...
void EmployeeService::fillDepartmentAndRole(shared_ptr<Employee> &employee) {
  ...
  if (department_id != 0) {
    employee->setDepartment(department_id,
                            department_dao_->getOne(department_id)->name());
  }
