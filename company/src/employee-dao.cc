...
shared_ptr<PageInfo<Employee>> EmployeeDao::getList(int page_no,
                                                    int page_size) {
  auto row_result =
      table_
          ->select("id", "username", "password", "name", "gender", "age",
                   "worked_time", "salary", "department_id", "role_id",
  ...
  return shared_ptr<PageInfo<Employee>>(
      new PageInfo<Employee>(page_no, page_size, table_->count(), employees));
}
...
int EmployeeDao::add(const Employee &employee) {
  int count =
      (int)table_
          ...
          .values(Util::string_to_utf8(employee.username()),
                  Util::string_to_utf8(employee.password()),
                  Util::string_to_utf8(employee.name()),
                  Util::string_to_utf8(employee.gender()), employee.age(),
                  employee.salary())
          .execute()
  ...
  if (employee.departmentId() != 0) {
    table_->update()
        .set("department_id", employee.departmentId())
        .where("`id`=" + to_string(employee.id()) + "and `deleted`<>true")
        .execute();
  };
  // 角色编号
...
int EmployeeDao::update(const Employee &employee) {
  return (int)table_->update()
      .set("username", Util::string_to_utf8(employee.username()))
      .set("password", Util::string_to_utf8(employee.password()))
      .set("name", Util::string_to_utf8(employee.name()))
      .set("gender", Util::string_to_utf8((employee.gender())))
      .set("age", employee.age())
      .set("worked_time", employee.workedTime())
...
int EmployeeDao::setDepartmentIdNull(int department_id) {
  return (int)table_->update()
      ...
      .where("`department_id`=" + to_string(department_id) +
             "and `deleted`<>true")
      .execute()
      .getAffectedItemsCount();
}
int EmployeeDao::setRoleIdNull(int role_id) {
  return (int)table_->update()
      .set("role_id", 1000)
      .where("`role_id`=" + to_string(role_id) + "and `deleted`<>true")
      .execute()
      .getAffectedItemsCount();
}

// "id", "username", "password", "name", "gender", "age", "worked_time",
// "salary", "department_id", "role_id", "create_time", "update_time"
