...
public:
	 ...
	 * @brief 返回带数据的结果
	*/
	static ResultEntity<T> successResultWidthData(const string &msg, const shared_ptr<T> &data) {
...
public:
	PageInfo(size_t no, size_t page_size, size_t size, const vector<shared_ptr<T>> &data)
		:no_(no), page_size_(page_size > size ? size : page_size), size_(size), data_(data) {
		pre_ = (no_ <= 0) ? 0 : no_ - 1;
		first_ = 0;
