...
ResultEntity<System> SystemService::login(string username, string password, const string &code) {
	...
	try {
		...
		return ResultEntity<System>::successResultWidthData("登录成功", system_dao_->login(username, password));
...
ResultEntity<System> SystemService::getSystemInfo() {
	...
	try {
		...
		return ResultEntity<System>::successResultWidthData("获取系统信息成功", system_dao_->getSystemInfo(system_info_->username()));
	} catch (const string &e) {
...
ResultEntity<string> SystemService::getVerificationCode(int length) {
	...
	try {
		...
		return ResultEntity<string>::successResultWidthData(GETCODE_SUCCESSED, shared_ptr<string>(new string(code)));
	} catch (const string &e) {
...
ResultEntity<int> SystemService::exportData(const string &filename) {
	...
	try {
		...
		return ResultEntity<int>::successResultWidthoutData(
			string("导出数据成功：").append(to_string(department_count))
			.append(" 条部门信息记录，").append(to_string(employees_count)).append(" 条员工信息记录"));
	} catch (const string &e) {
