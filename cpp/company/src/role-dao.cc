...
shared_ptr<PageInfo<Role>> RoleDao::getList(int page_no, int page_size) {
	...
	for (auto row : row_result) {
		roles.push_back(shared_ptr<Role>(new Role(row[0], Util::utf8_to_string(row[1].get<string>()))));
	}
	return shared_ptr<PageInfo<Role>>(new PageInfo<Role>(page_no, page_size, table_->count(), roles));
}

string RoleDao::getRoleName(int role_id) {
	auto row_result = table_->select("name").where("`id`=" + to_string(role_id)).execute();
...
double RoleDao::getMonthSalary(int employee_id, int role_id) {
	auto row_result = table_->select("month_salary").where("`id`=" + to_string(role_id)).execute();
	if (row_result.count() <= 0) {
		throw string("不存在该角色");
	}
	...
	auto result = table_->getSession().sql(Util::utf8_to_string(s.get<string>())).bind("employee_id", employee_id).execute();
	if (result.count() <= 0) {
		throw string("不存在该员工");
	}
