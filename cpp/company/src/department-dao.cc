...
shared_ptr<PageInfo<Department>> DepartmentDao::getList(int page_no, int page_size) {
	auto row_result = table_->select("id", "name", "manager_id", "sales").where("`deleted`<>true")
		.limit(page_size).offset((page_no - 1) * page_size).execute();
	vector<shared_ptr<Department>> departments;
	for (auto row : row_result) {
		departments.push_back(fill(row));
	}
	return shared_ptr<PageInfo<Department>>(new PageInfo<Department>(page_no, page_size, table_->count(), departments));
}

shared_ptr<Department> DepartmentDao::getOne(int id) {
	auto row_result = table_->select("id", "name", "manager_id", "sales").where("`id`=" + to_string(id) + " and `deleted`<>true").execute();
...
int DepartmentDao::add(const Department &department) {
	int count = (int)table_->insert("name", "sales").values(Util::string_to_utf8(department.name()), department.sale()).execute().getAffectedItemsCount();
	if (department.managerId() != 0) {
		string name = department.name();
		table_->update().set("manager_id", department.managerId()).where("`name`=" + Util::string_to_utf8(Util::quote(name)) + " and `deleted`<>true").execute();
	};
	return count;
}
...
int DepartmentDao::update(const Department &department) {
	auto up = table_->update().set("name", Util::string_to_utf8(department.name())).set("sales", department.sale());
	if (department.managerId() >= 10000) {
		up = up.set("manager_id", department.managerId());
	}
	return (int)up.where("`id`=" + to_string(department.id()) + " and `deleted`<>true").execute().getAffectedItemsCount();
}

int DepartmentDao::setManagerIdNull(int manager_id) {
	return (int)table_->getSession()
		.sql("update `department` set `manager_id` := null where `manager_id`=" + to_string(manager_id)).execute().getAffectedItemsCount();;
}
