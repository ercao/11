...
ResultEntity<PageInfo<Department>> DepartmentService::getPageInfo(string page_no, string page_size) {
	...
	try {
		...
		for (auto department : result->data_) {
			if (department->managerId() > 0) {
				department->setName(employee_dao_->getOne(department->managerId())->name());
			}
		}
		return ResultEntity<PageInfo<Department>>::successResultWidthData("成功获取到" + to_string(result->data_.size()) + "条数据", result);
	} catch (const string &e) {
		return ResultEntity<PageInfo<Department>>::faildResult(GETLIST_FAILD_PREFIX + e);
	} catch (const exception &e) {
		return ResultEntity<PageInfo<Department>>::faildResult(GETLIST_FAILD_PREFIX + e.what());
	}
}
...
ResultEntity<Department> DepartmentService::getOne(string id) {
	...
	try {
		...
		return ResultEntity<Department>::successResultWidthData("获取数据成功", department);
	} catch (const string &e) {
		return ResultEntity<Department>::faildResult(GETONE_FAILD_PREFIX + e);
	} catch (const exception &e) {
		return ResultEntity<Department>::faildResult(GETONE_FAILD_PREFIX + e.what());
	}
...
ResultEntity<int> DepartmentService::edit(Department &department) {
	...
	try {
		...
		if (department.managerId() < 10000 || nullptr == employee_dao_->getOne(department.managerId())) {
