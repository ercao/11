#import "@preview/tablex:0.0.6": tablex, hlinex, colspanx, rowspanx
...
#dataset.map(((title, gammer, tests)) => [
  ...
  #[
    ...
    #figure(caption: [#LR1 项目集表], kind: table,
      table(
        columns: 3,
        align: top + left,
        stroke: 0.5pt,

        ..lr1.items.enumerate().map(((index, item)) => {
          let (leader, p) = item
    ...
    #figure(caption: [#LR1 预测分析表], kind: table,
      tablex(
        ...
        rowspanx(2)[状态], colspanx(gammer.t.len() + 1, to-math("ACTION")), colspanx(gammer.nt.len(), to-math("GOTO")),

        ..gammer.t.map(math.equation), to-math("#"), ..gammer.nt.map(math.equation),

        ..lr1.analysis-table.enumerate().map(((index, row)) => {
          (index, )

          row.enumerate().map(((index, col)) => {

            set text(red) if col.len() > 1
            col.map(action => {
              if index <= gammer.t.len() {
                if action == none { }
                else if action == 0 { to-math("ACC") }
                else if action.at(0) == 1 { $S_#to-math(str(action.at(1)))$ }
    ...
    #[
      ...
      #tests.map(s => {
        ...
        figure(caption: [#to-math(s) 的 #LR1 分析过程], kind: table,
          tablex(
            columns: 6,
            align: center + horizon,
            ...
            map-cells: cell => {
              if cell.y == 0 {
                ...
                (..cell, align: (center, left, left, right, center, center))
              }
            },

            [步骤], [状态栈], [符号栈], [输入串], to-math("ACTION"), to-math("GOTO"),
            ..steps.enumerate().map(((index, row)) => {
              (
                index + 1,
        ...
        if syntex-tree != none {
          figure(caption: [#s 的 #LR1 语法树],
            cetz.canvas({
              import cetz.draw: *

              cetz.tree.tree(syntex-tree,
                content: (padding: .1),
                line: (stroke: 0.5pt),
                draw-node: (node, _) => {
