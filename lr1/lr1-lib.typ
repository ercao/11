#let build(origin-gammer) = {
  ...
  let analysis-table = {
    ...
    for (from, tos) in dfa.pairs() {
      ...
      let index = map.at(x)
        analysis-table.at(int(from)).at(index).push(
          if index <= gammer.t.len() { (1, to) } else { to }
        )
      }
    }
